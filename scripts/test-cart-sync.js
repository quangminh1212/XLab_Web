...
async function updateUserCart(cartData) {
  try {
    ...
    console.log('   Total quantity:', cartData.reduce((sum, item) => sum + item.quantity, 0));
    console.log('   Total value:', cartData.reduce((sum, item) => sum + (item.price * item.quantity), 0).toLocaleString('vi-VN') + ' VND');
    
    // Verify data
    const verifyData = JSON.parse(fs.readFileSync(USER_FILE, 'utf8'));
    console.log('\n🔍 Verification:');
    console.log('   Cart saved correctly:', verifyData.cart.length === cartData.length ? '✅' : '❌');
    console.log('   Timestamp updated:', verifyData.metadata.lastUpdated !== userData.metadata.lastUpdated ? '✅' : '❌');
