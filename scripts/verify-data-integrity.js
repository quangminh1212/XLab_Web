...
function validateUserData(userData, email) {
  ...
  if (userData.profile) {
    ...
    if (!isValidISO8601(userData.profile.createdAt)) errors.push('Invalid createdAt timestamp format');
    if (!isValidISO8601(userData.profile.updatedAt)) errors.push('Invalid updatedAt timestamp format');
    if (userData.profile.lastLogin && !isValidISO8601(userData.profile.lastLogin)) errors.push('Invalid lastLogin timestamp format');
  }
  
  // Validate metadata
  if (userData.metadata) {
    if (!isValidISO8601(userData.metadata.lastUpdated)) errors.push('Invalid metadata.lastUpdated timestamp format');
  ...
  if (userData.settings) {
    if (typeof userData.settings.notifications !== 'boolean') errors.push('Invalid settings.notifications type');
