...
function safeCall(fn: Function, fnName: string, ...args: any[]) {
  try {
    console.log(`DEBUG: Safely calling ${fnName}`);
    return fn(...args);
  } catch (error) {
    console.error(`DEBUG: Error in ${fnName}:`, error);
    return null;
  }
}
...
export default function Header() {
  ...
  // Wrap useSession in try/catch to diagnose issues
  let session;
  try {
    const { data, status } = useSession();
    console.log('DEBUG: useSession status:', status);
    session = data;
  } catch (error) {
    console.error('DEBUG: Error in useSession:', error);
    session = null;
  }
  ...
  useEffect(() => {
    console.log('DEBUG: Header useEffect running');
    ...
    const handleScroll = () => {
      try {
        ...
        console.error('DEBUG: Error in scroll handler:', error);
      }
    }

    try {
      // Check all objects are properly defined
      console.log('DEBUG: window object status:', typeof window !== 'undefined' ? 'defined' : 'undefined');
      
      if (typeof window !== 'undefined') {
        window.addEventListener('scroll', handleScroll);
        
        // Track initial scroll position
        console.log('DEBUG: Initial scroll position:', window.scrollY);
      }
    } catch (error) {
      console.error('DEBUG: Error setting up scroll listener:', error);
    }
    ...
    return () => {
      try {
        ...
        console.error('DEBUG: Error cleaning up scroll listener:', error);
      }
    };
  }, [language])
  ...
  // Use safe versions of functions that might cause issues
  const handleMobileMenuToggle = () => {
    safeCall(() => setMobileMenuOpen(!mobileMenuOpen), 'handleMobileMenuToggle');
  };
  ...
  try {
    ...
    console.error('DEBUG: Error rendering Header component:', error);
    // Return a simplified header that won't crash
    return (
      <header className="bg-white shadow-sm">
        <div className="container mx-auto px-4 py-4">
          <div className="flex justify-between items-center">
            <Link href="/" className="text-xl font-bold">XLab</Link>
            <div>
              <span className="text-red-600">Error loading header</span>
            </div>
          </div>
        </div>
      </header>
