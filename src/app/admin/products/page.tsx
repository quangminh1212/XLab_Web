...
function AdminProductsPage() {
  ...
  const fetchProducts = useCallback(async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(localCode === 'vi' ? 'Không thể tải danh sách sản phẩm' : 'Could not load products list');
        }
        ...
        console.error(localCode === 'vi' ? 'Lỗi khi tải danh sách sản phẩm:' : 'Error loading products list:', err);
      } finally {
        setLoading(false);
      }
  ...
  const handleDelete = async (productId: string) => {
    if (confirm(localCode === 'vi' ? 'Bạn có chắc chắn muốn xóa sản phẩm này?' : 'Are you sure you want to delete this product?')) {
      try {
        setLoading(true);
        ...
        if (!response.ok) {
          throw new Error(localCode === 'vi' ? 'Không thể xóa sản phẩm' : 'Could not delete product');
        }

        // Remove the product from the list
        setProductList(productList.filter((product) => product.id !== productId));
        alert(localCode === 'vi' ? 'Đã xóa sản phẩm thành công!' : 'Product deleted successfully!');
      } catch (err) {
        setError((err as Error).message);
        console.error(localCode === 'vi' ? 'Lỗi khi xóa sản phẩm:' : 'Error deleting product:', err);
      } finally {
        setLoading(false);
      }
    }
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b pb-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">
            {localCode === 'vi' ? 'Quản lý sản phẩm' : 'Product Management'}
          </h1>
          <p className="text-gray-500 text-base mt-1">
            {localCode === 'vi' ? 'Quản lý danh sách sản phẩm của cửa hàng' : 'Manage your store products list'}
          </p>
        </div>
        <Link
          href="/admin"
        ...
        <div className="p-5 border-b border-gray-100">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div className="relative w-full md:w-96">
              <input
                type="text"
                placeholder={localCode === 'vi' ? "Tìm kiếm sản phẩm..." : "Search products..."}
                value={searchTerm}
            ...
            >
              ...
              {localCode === 'vi' ? 'Thêm sản phẩm mới' : 'Add new product'}
            </Link>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="p-8 text-center bg-gray-50">
            ...
            <p className="text-gray-500 text-xl">
              {localCode === 'vi' ? 'Không tìm thấy sản phẩm nào' : 'No products found'}
            </p>
                      ...
                      <div className="text-base font-medium text-gray-900">
                        {(() => {
                          ...
                          if (minPrice === Infinity) {
                            return localCode === 'vi' ? 'Chưa có giá' : 'No price set';
                          }
                    ...
                    <td className="px-3 py-4 whitespace-nowrap text-center">
                      <div className="flex justify-center space-x-2">
                        <button
                          ...
                          onClick={() => handleEdit(product.id)}
                          title={localCode === 'vi' ? "Sửa sản phẩm" : "Edit product"}
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            className="h-5 w-5"
                        ...
                        <button
                          ...
                          title={localCode === 'vi' ? "Xóa sản phẩm" : "Delete product"}
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            className="h-5 w-5"
