...
// Thêm khai báo interface cho window để TypeScript hiểu được thuộc tính google
declare global {
  interface Window {
    google?: {
      accounts: {
    ...
    const initializeGoogleIdentity = () => {
      if (typeof window !== 'undefined' && window.google) {
        try {
          ...
          console.error('Lỗi khởi tạo Google Identity:', error);
        }
      }
    };

    // Gọi sau khi component đã được render
  ...
  const handleGoogleResponse = async (response: any) => {
    try {
      ...
      if (response?.credential) {
        // Sử dụng credential từ Google để đăng nhập vào NextAuth
      ...
      setError('Có lỗi xảy ra khi đăng nhập với Google. Vui lòng thử lại sau.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="min-h-screen flex flex-col justify-center py-12 sm:px-6 lg:px-8 bg-gradient-to-b from-white to-gray-50">
        ...
        <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
          <div className="bg-white py-8 px-4 shadow-xl sm:rounded-lg sm:px-10 border border-gray-100">
            ...
            <div id="google-login-button" className="mb-6"></div>
