...
export default function ProductDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    async function loadProduct() {
      try {
        if (!params || !params.id) {
          setError(isLoaded ? translate('messages.invalidProductId') : 'Mã sản phẩm không hợp lệ');
          setLoading(false);
          return;
        }
        ...
        if (foundProduct) {
          ...
          document.title = isLoaded
            ? `${foundProduct.name} | XLab`
            : `${foundProduct.name} | XLab - Phần mềm và Dịch vụ`;
        } else {
          setError(isLoaded ? translate('messages.productNotFound') : 'Không tìm thấy sản phẩm');
        }
      } catch (err) {
        console.error('Error loading product:', err);
        setError(isLoaded ? translate('messages.errorLoadingProduct') : 'Đã xảy ra lỗi khi tải thông tin sản phẩm');
      } finally {
        setLoading(false);
      }
    }
    ...
    return (
      <div className="py-12 flex justify-center">
        ...
        <p className="ml-3 text-gray-600">
          {isLoaded ? translate('loading.message') : 'Đang tải...'}
        </p>
      </div>
    );
  }
      ...
      <div className="py-8 text-center">
        <div className="container mx-auto px-4">
          <div className="max-w-2xl mx-auto bg-white p-8 rounded-lg shadow-md">
            ...
            <h1 className="text-2xl font-bold text-red-600 mb-4">{error || (isLoaded ? translate('messages.error') : 'Đã xảy ra lỗi')}</h1>
            <p className="text-gray-600 mb-6">
              {isLoaded
                ? translate('messages.productNotFoundDescription')
                : 'Sản phẩm bạn đang tìm kiếm không tồn tại hoặc đã bị xóa. Vui lòng kiểm tra lại đường dẫn hoặc quay lại trang sản phẩm.'}
            </p>
            <div className="flex flex-col sm:flex-row justify-center gap-4">
              ...
              >
                ...
                {isLoaded ? translate('actions.backToProductList') : 'Quay lại danh sách sản phẩm'}
              </Link>
            </div>
          </div>
        </div>
  ...
  const getCategoryTranslation = (categorySlug?: string) => {
    ...
    if (translationKey && isLoaded) {
      return translate(`categories.categoriesList.${translationKey}.name`);
    }
            ...
            <div className="flex flex-col md:flex-row -mx-4">
              <div className="md:flex-1 px-4 mb-6 md:mb-0">
                ...
                <div className="mt-6 flex items-center justify-between">
                  <div className="flex items-center text-sm text-gray-500">
                    ...
                    {product.viewCount || 0} {isLoaded ? translate('product.views') : 'lượt xem'}
                  </div>
                  ...
                  <div className="flex items-center text-sm text-gray-500">
                    ...
                    {product.downloadCount || 0} {isLoaded ? translate('product.downloads') : 'lượt tải'}
                  </div>
                </div>
              </div>
              ...
              <div className="md:flex-1 px-4">
                ...
                <p className="text-gray-600 text-sm mb-4">
                  {isLoaded ? translate('product.version') : 'Phiên bản'} {product.version || '1.0'} |
                  {isLoaded ? translate('product.updated') : 'Cập nhật'}: {new Date(product.updatedAt || new Date()).toLocaleDateString('vi-VN')}
                </p>
                ...
                <div className="mb-6">
                  <div className="flex space-x-4">
                    ...
                    >
                      ...
                      {isLoaded ? translate('actions.download') : 'Tải xuống'}
                    </button>

                    <Button
                      variant="outline"
                      className="flex-1 py-3 border-2"
                      onClick={() => alert(isLoaded ? translate('messages.featureInDevelopment') : 'Tính năng này đang được phát triển')}
                    >
                      <svg className="w-5 h-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                      </svg>
                      {isLoaded ? translate('actions.addToCart') : 'Thêm vào giỏ hàng'}
                    </Button>
                  </div>
                </div>
                ...
                <div className="border-t border-gray-200 pt-6">
                  <h3 className="text-lg font-semibold mb-4">
                    {isLoaded ? translate('product.information') : 'Thông tin sản phẩm'}
                  </h3>
                  <ul className="space-y-3">
                    <li className="flex justify-between">
                      <span className="text-gray-600">
                        {isLoaded ? translate('product.licenseType') : 'Loại bản quyền:'}
                      </span>
                      <span className="font-medium">{product.licenseType || 'Thương mại'}</span>
                    </li>
                    <li className="flex justify-between">
                      <span className="text-gray-600">
                        {isLoaded ? translate('product.size') : 'Kích thước:'}
                      </span>
                      <span className="font-medium">{product.size || '10MB'}</span>
                    </li>
                    <li className="flex justify-between">
                      <span className="text-gray-600">
                        {isLoaded ? translate('product.releaseDate') : 'Ngày phát hành:'}
                      </span>
