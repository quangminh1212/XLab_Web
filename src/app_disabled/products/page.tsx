...
export default function ProductsPage() {
  ...
  useEffect(() => {
    document.title = isLoaded
      ? `${translate('navigation.products')} | XLab`
      : 'Sản phẩm | XLab - Phần mềm và Dịch vụ'
      ...
      <div className="py-12 flex justify-center">
        <div className="flex flex-col items-center">
          ...
          <p className="text-gray-600">
            {isLoaded ? translate('loading.message') : 'Đang tải sản phẩm...'}
          </p>
        </div>
      </div>
    )
      ...
      <div className="py-12 flex justify-center">
        <div className="max-w-2xl mx-auto bg-white p-8 rounded-lg shadow-md text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-4">
            {isLoaded ? translate('messages.error') : 'Không thể tải sản phẩm'}
          </h2>
          ...
          <div className="flex justify-center gap-4">
            ...
            >
              ...
              {isLoaded ? translate('messages.tryAgain') : 'Thử lại'}
            </button>
            <Link
              href="/"
            ...
            >
              ...
              {isLoaded ? translate('navigation.home') : 'Về trang chủ'}
            </Link>
          </div>
        </div>
      </div>
      ...
      <div className="py-12">
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mx-auto bg-white p-8 rounded-lg shadow-md text-center">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-4">
              {isLoaded ? translate('messages.productComingSoon') : 'Chưa có sản phẩm'}
            </h2>
            <p className="text-gray-600 mb-8">
              {isLoaded ? translate('messages.noProductsYet') : 'Hiện tại chưa có sản phẩm nào được thêm vào hệ thống. Bạn có thể thêm sản phẩm mới bằng cách nhấn vào nút bên dưới.'}
            </p>
            <div className="flex justify-center">
              <Link href="/admin/products/new">
                <Button className="flex items-center gap-2">
                  ...
                  {isLoaded ? translate('actions.addNewProduct') : 'Thêm sản phẩm mới'}
                </Button>
              </Link>
            </div>
          </div>
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm mb-8">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            {(searchTerm || selectedCategory) && (
              ...
              >
                ...
                {isLoaded ? translate('actions.clearFilters') : 'Xóa bộ lọc'}
              </button>
            )}
          </div>
          ...
          <div className="mt-4">
            <h3 className="text-sm font-medium text-gray-700 mb-2">
              {isLoaded ? translate('navigation.categories') + ':' : 'Danh mục:'}
            </h3>
        ...
        <div className="mb-12">
          <h2 className="text-xl font-bold text-gray-900 mb-4">
            {searchTerm || selectedCategory
              ? (isLoaded ? translate('actions.searchResults') : 'Kết quả tìm kiếm')
              : (isLoaded ? translate('footer.allProducts') : 'Tất cả sản phẩm')}
            {filteredProducts.length > 0 && (
              <span className="text-gray-500 text-base font-normal ml-2">
                ({filteredProducts.length} {isLoaded ? translate('categories.products') : 'sản phẩm'})
              </span>
            )}
          </h2>
          <ProductGrid
            ...
            emptyMessage={
              searchTerm || selectedCategory
                ? (isLoaded ? translate('messages.noSearchResults') : "Không tìm thấy sản phẩm nào phù hợp với bộ lọc đã chọn.")
                : (isLoaded ? translate('messages.noProducts') : "Chưa có sản phẩm nào.")
            }
          />
        </div>
